...
def paraclust(texts,query):
    ...
    max_iter = 300
    number_of_process=1
    model = KMeans(n_clusters=n_clusters, max_iter=max_iter, n_init=number_of_seeds_to_try, n_jobs=number_of_process,random_state=1234).fit(X)
...
#         filename = ntpath.basename(doc_id)
#         head, tail = os.path.split(doc_id)
#         # print(filename)
